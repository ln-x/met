...
for i in ZAMGnames:
  ...
  ZAMG_heatdays_VP = ZAMG_heatdays_VP
  ...
  '''calculation of bias'''
  #Bias_Forc_i = np.average(WRFdata_heatdays_-ZAMG_heatdays)
  ...
  #  ax2.legend(loc='lower right')
    ...
    # plt.scatter(ZAMG_heatdays, WRFdata_heatdays, color='blue', label=(r"$R^2$=%.2f, Bias=%.2f" % (R2_Forc_i, Bias_Forc_i)))#, s=3, label=u"STQ,  R²=0.92")  #squared= u"\u00B2"?
    # plt.scatter(ZAMG_heatdays, WRFdata2_heatdays, color='violet', label=(r"$R^2$=%.2f, Bias=%.2f" % (R2_Forc_i2, Bias_Forc_i2)))#, s=3, label=u"STQ,  R²=0.92")  #squared= u"\u00B2"?
    ax2.set_xlabel(r"Water vapour pressure (observed) "u'[hPa]', size="medium")
    ax2.set_ylabel(r"Water vapour pressure (simulated) "u'[hPa]', size="medium")
    ax2.legend(loc='lower right')
    plt.suptitle(ZAMGnames[i][0], size="large")#+"2m air temperature"))
    ax2.set_xlim(11, 27)
    ax2.set_ylim(11, 27)
