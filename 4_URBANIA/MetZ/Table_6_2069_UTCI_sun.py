...
def collectMAX(relSIMPATH):
    ...
    var_reg = f.variables['UTCI_OUTSUN'][126:174, a1:a2, b1:b2]  #126:174
    for i in range(9):
        for j in range(9):
           add = (var_reg[:,i,j].max())
           if add!=nan:
    ...
    #rint "\n",relSIMPATH, "max_diff ", np.mean(list(filter(lambda x: x != 0, out1))) -  45.2181257379
    #print relSIMPATH, "min_diff ", np.mean(list(filter(lambda x: x != 0, out2))) - 28.7547362824
