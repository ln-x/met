...
file_ref2015 = '/media/lnx/Norskehavet/OFFLINE/2015REF/dx345corr/SURF_ATM_DIAGNOSTICS.OUT.nc'
#dimensions(sizes): xx(174), yy(135), time(174) 1.7.2069 18h - 8.7.23h

f_ref2015 = Dataset(file_ref2015, mode='r')

var_units = f_ref2015.variables['T2M'].units
...
"""
a1,a2,b1,b2= 51,60,81,90 #50,59,80,89   #CE 51,60,81,90
...
'''cut for subregions'''
var_ref = f_ref2015.variables['T2M']
#print len(f_ref2015.variables['UTCI_OUTSHAD'])
#print f_ref.variables['time']#[150:174]

var_ref2015_ce = f_ref2015.variables['T2M'][150:198,a1:a2,b1:b2]
...
for i in range(9):
    for j in range(9):
        ...
        add = val1 #np.maximum(val1,val2)
        if add!=nan:
          utci_ref2015_ce_max.append(add)
        else:
          utci_ref2015_ce_max.append(0)
        val3= var_ref2015_ce[:,i,j].min()
        #val4 = var_ref2015_ce2[:,i,j].min()
        add2 = val3 #np.minimum(val3,val4)
        if add2!= nan:
...
def collectMAX(relSIMPATH):
    out1=[]   #max
    out2=[]   #min
    file = '/media/lnx/Norskehavet/OFFLINE/'+relSIMPATH+'/SURF_ATM_DIAGNOSTICS.OUT.nc'
    f = Dataset(file, mode='r')
    var_reg = f.variables['T2M'][126:174, a1:a2, b1:b2]
    for i in range(9):
        for j in range(9):
           add = (var_reg[:,i,j].max())
           if add!=nan:
    ...
    print "\n",relSIMPATH, "max_diff ", np.mean(list(filter(lambda x: x != 0, out1))) - 316.884964468  #SA: - 315.135548145 #CE: - 316.884964468 #43.9708595711
    print relSIMPATH, "min_diff ", np.mean(list(filter(lambda x: x != 0, out2))) -302.926024338 #SA: - 298.074509641 #CE: -302.926024338 #26.9300127078
