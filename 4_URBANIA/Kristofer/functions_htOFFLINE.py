...
def collect_data(date, para, cuttime, region, region_sn, region_we, path):
    ...
    filename = 'TEB_DIAGNOSTICS.OUT.nc'
    ...
    ref_filepath = path + '2069REF/dx345corr/'
    spr_filepath = path + '2069SPR/'
    opt_filepath = path + '2069OPT/dx345corr/'

    #print ref_filepath + filename
    #exit()

    ref_ncfile = Dataset(ref_filepath + filename)
    spr_ncfile = Dataset(spr_filepath + filename)
    opt_ncfile = Dataset(opt_filepath + filename)
    wrf_ncfile = Dataset('/media/lnx/Norskehavet/WRF-TEB-NC_Files/FINAL/WRF-2019-Runs/REF_Run_2069/wrfout_d03_2069-07-01_18_00_00.nc')

    #times = getvar(ref_ncfile, "Times", timeidx=wrf.ALL_TIMES, meta=False) # FC times in np.datetime64 format - very ugly
    times = getvar(wrf_ncfile, "Times", timeidx=wrf.ALL_TIMES, meta=False) # FC times in np.datetime64 format - very ugly
