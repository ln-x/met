...
start = MAM19_s
end = datetime(2019, 6, 1, 00, 00)

start2 = datetime(2019, 4, 15, 00, 00)
end2 = datetime(2019, 6, 1, 00, 00)
...
ax2.plot(o3_1990_2020_mda8_w['AT9STEF'][start:end].index, o3_1990_2020_mda8_w['AT9STEF'][datetime(2018, 3, 1):datetime(2018, 6, 1)].values,linewidth="1", color='violet', linestyle=":", label="O3_reg")
...
ax1.plot(BOKUMetData_weekly["GR"][start:end].index, (BOKUMetData_weekly["GR"][datetime(2018, 3, 1):datetime(2018, 6, 1)].values)/1000, linewidth="1", color='orange', linestyle=":", label="GR_reg")
...
ax1.plot(tsif_w[start:datetime(2019, 6, 1)].index, osif_757_w[datetime(2018, 3, 1):datetime(2018, 6, 1)].values,color='red',label="SIF_reg (OCO)",linestyle=":")
ax1.set_ylabel("[mW/m2/sr/nm]", size="medium")
ax2.plot(fAPAR[start:datetime(2019, 6, 1)],color='green', label="fAPARa_dry")#label="fAPAR anomaly"
ax2.plot(fAPAR[start:datetime(2019, 6, 1)].index, fAPAR[datetime(2018, 3, 1):datetime(2018, 6, 1)].values,color='green', linestyle=":", label="fAPARa_reg")
ax2.set_ylabel("[-]", size="medium")
ax1.grid()
ax1.set_xticks([])
ax1.set_xlim(start,end)
...
ax2.plot(vpd_dmax_w[start:end].index, vpd_dmax_w[datetime(2018, 3, 1):datetime(2018, 6, 1)].values, linewidth="1", color='green', linestyle=":", label="VPD_reg")
...
ax1.plot(hcho_w[start:end].index,hcho_w[datetime(2018, 3, 1):datetime(2018, 6, 1)].values, linewidth="1", color='black', linestyle=":", label="HCHO_reg")
ax1.set_xlim(start,end)
ax1.set_xticks([])
ax2.set_ylabel("[kPa]", size="medium")
ax1.set_ylabel("[ppb]", size="medium")
...
ax2.plot(BOKUMetData_weekly["AT"][start:end].index,BOKUMetData_weekly["AT"][MAM18_s:datetime(2018, 6, 1)].values, linewidth="1", color='red', linestyle=":", label="Tair_reg")
#ax1.plot(hcho_d[start:end], linewidth="0.3", color='black')#, label="HCHO") #,dmax", linestyle="solid")
ax1.plot(hcho_w[start:end], linewidth="1", color='black',label="HCHO_dry")#, label="HCHO,dmax,w", linestyle="solid")
#ax1.plot(hcho_d[start:end].index,hcho_d[MAM18_s: datetime(2018, 6, 1)].values, linewidth="0.3", color='black', linestyle=":")
ax1.plot(hcho_w[start:end].index,hcho_w[MAM18_s: datetime(2018, 6, 1)].values, linewidth="1", color='black', linestyle=":",label="HCHO_dry")
ax1.set_xlim(start,end)
#ax2.axvline(x=datetime(2020,5,10))
