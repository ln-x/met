...
#print(Emis_ol_noontime20[start:end])
#exit()
...
ax2.plot(o3_1990_2020_mda8_w['AT9STEF'][start:end].index, o3_1990_2020_mda8_w['AT9STEF'][datetime(2018, 3, 1):datetime(2018, 6, 7)].values,linewidth="1", color='violet', linestyle=":", label="O3_ref")
...
ax1.plot(BOKUMetData_weekly["GR"][start:end].index, (BOKUMetData_weekly["GR"][datetime(2018, 3, 1):datetime(2018, 6, 7)].values)/1000, linewidth="1", color='orange', linestyle=":", label="GR_ref")
...
ax2.plot(fAPAR[start:datetime(2020, 6, 1)].index, fAPAR[datetime(2018, 3, 1):datetime(2018, 6, 7)].values,color='green', linestyle=":", label="fAPARa_ref")
ax2.set_ylabel("[-]", size="medium")
ax1.grid()
ax1.set_xticks([])
ax1.set_xlim(start,end)
...
ax1.plot(vwc['RSS_sub_wWheat'][start:end].index,vwc['RSS_sub_wWheat'][datetime(2018, 3, 1):datetime(2018, 6, 1)].values,linewidth="1", color='orange', linestyle=":", label="RSS_sub_w_ref")
...
ax2.step(BOKUMetData_dailysum[start:end].index,(BOKUMetData_dailysum["PC"]*0.1)[datetime(2018, 3, 1):datetime(2018, 6, 1)].values, linewidth="0.3", color='blue', linestyle=":", label="PR_ref") #{'pre', 'post', 'mid'}  label="PR,sum",
ax2.axhline(0, color='grey',linestyle="dashed",linewidth="0.3")
ax2.set_ylabel("[mm]", size="medium")
ax1.set_ylabel("[-]", size="medium")
ax1.set_xticks([])
...
ax2.plot(vpd_dmax_w[start:end].index, vpd_dmax_w[datetime(2018, 3, 1):datetime(2018, 6, 7)].values, linewidth="1", color='green', linestyle=":", label="VPD_ref")
...
ax1.plot(hcho_w[start:end].index,hcho_w[datetime(2018, 3, 1):datetime(2018, 6, 7)].values, linewidth="1", color='black', linestyle=":", label="HCHO_ref")
ax1.set_xlim(start,end)
ax1.set_xticks([])
ax2.set_ylabel("[kPa]", size="medium")
ax1.set_ylabel("[ppb]", size="medium")
...
ax2.plot(Emis_assim_noontime20[start:end], linewidth="2", color='grey', label="ISO_dry_mod") #label="GR,sum,w"
ax2.plot(Emis_assim_noontime20[start:end].index,Emis_assim_noontime18[MAM18_s:datetime(2018, 6, 1)].values, linewidth="2", color='grey', linestyle=":", label="ISO_ref_mod")
