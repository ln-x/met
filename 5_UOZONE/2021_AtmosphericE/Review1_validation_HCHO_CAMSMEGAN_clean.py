...
#f_ol_201907 = netCDF4.Dataset(file_ol_2, mode='r')
...
#start2 = datetime.datetime(2019,7,1,0,0) #hours since 2019-07-01 00:00:00
#stop2 = datetime.datetime(2019,8,1,0,0)
#dts2 = list(rrule.rrule(rrule.HOURLY,count=744,dtstart=start))
#print(dts)
#ISO_20190607_mol = pd.concat(ISO_201906_mol,ISO_201907_mol)
ISO_20190607_mol = np.ma.append(ISO_201906_mol, ISO_201907_mol)
#print(len(ISO_20190607_mol))
...
Emis_ol2_day = Emis_ol2.loc[(Emis_ol2.index.hour >=9)&(Emis_ol2.index.hour <=15)]
print(Emis_ol2_day)
exit()
...
ax2 = ax1.twinx()
ax1.plot(Emis_ol,linewidth="1", color='black', label="MOD: MGN3,ol ISO,dmax", linestyle="solid")
ax1.plot(Emis_assim,linewidth="1", color='red', label="MOD: MGN3,assim ISO,dmax", linestyle="solid")
ax2.plot(hcho_dmax[:],linewidth="1", color='black', label="OBS: HCHO,dmax", linestyle=":")
ax1.plot(Emis_ol2,linewidth="1", color='blue', label="MOD: MGN2.1,ol ISO,dmax", linestyle="solid")
ax1.set_xlabel("days")
...
def Plot6var(df1, df2, title, ylimit):
    ...
    hcho_JJA2019 = df2[datetime.datetime(2019, 6, 1, 00, 00): datetime.datetime(2019, 7, 31, 00, 00)].values.flatten()
    Emis_JJA2019 = df1[datetime.datetime(2019, 6, 1, 00, 00): datetime.datetime(2019, 7, 31, 00, 00)].values.flatten()
    ...
    #print(Emis_ol2.values)
    Emis_olnp = Emis_ol2.to_numpy().flatten()
    idx2 = np.isfinite(Emis_olnp) & np.isfinite(hcho_JJA2019)
    SRho2, Sp2 = (stats.spearmanr(Emis_olnp[idx], hcho_JJA2019[idx]))
    m52, b52 = np.polyfit(Emis_olnp[idx], hcho_JJA2019[idx], 1)
    ...
    plt.plot(Emis_JJA2019, y_est, color=color2)
