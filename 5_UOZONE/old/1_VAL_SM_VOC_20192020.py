...
ax2.plot(nox_1990_2020_da['AT9STEF'][start:end]*ugm3toppb_o3,linewidth="1", color='blue', label="nox OBS da", linestyle="solid")
...
##Full period
pff = pd.concat([hcho_dmax,vwc["RSS_top_wWheat"],BOKUMetData_dailymax["AT"],BOKUMetData_dailysum["GR"]],axis=1, keys=['hcho','SM','AT','GR'])
print(pff[:])
pff5 = pff[:].dropna()
#pff4.columns = pff4.columns.droplevel(-1)
#isHighGRall = pff4["GR"] > 15000 #15 KWh (per day) Globalstrahlung
