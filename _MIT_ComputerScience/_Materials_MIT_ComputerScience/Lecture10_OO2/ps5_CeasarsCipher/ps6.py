    ...
    def build_shift_dict(self, shift):
        ...
        for i in range(len(alphabet2)):
            if (i + shift + 1) >= len(alphabet2):
                shifted.append(alphabet2[i-len(alphabet2) + shift])
                shifted2[alphabet2[i]] = alphabet2[i-len(alphabet2)+shift]
            else:
                shifted.append(alphabet2[i + shift])
...
"""
def decrypt_story():
    word_list = get_story_string()
    encryptedstory = CiphertextMessage(word_list)
